...
def main():
    ...
    fileContent = SpeechSettings.fileSettingsRead()
    ...
    """
    engine = AudioLibrary.speechEngineSelection()
    ...
    voiceCommandKey = SpeechSettings.commandValidation(fileContent, userCommandArray[0])

    #Execute asked command
    if voiceCommandKey == True:
        SpeechSettings.commandExecution(fileContent, engine, userCommandArray)
    ...
    try:
        with sr.Microphone() as source:
            if fileContent.get('settings').get('debug') == 'on':
                ...
                return SpeechSettings.commandDebugOn(fileContent)
            elif fileContent.get('settings').get('debug') == 'off':
                print('Debug option has been turned off')
                time.sleep(1)
                return SpeechSettings.commandDebugOff(fileContent, source)
            else:
